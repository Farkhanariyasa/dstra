...
for (let i = 1; i <= 2; i++) {
  document.getElementById("unduh-t1-" + [i]).addEventListener("click", (e) => {
    idbutton = "btndownload" + [i];
    console.log("t1-" + [i] + " diklik");
    idinput = i - 1;
  });
}
...
for (let i = 1; i <= 2; i++) { 
  document
    ...
    .addEventListener("click", function (e) {
      ...
      let canvasUrl = document.getElementById("riset2_t1_hasil"+[i]).toDataURL();
      // Create an anchor, and set the href value to our data URL
      const createEl = document.createElement("a");
      createEl.href = canvasUrl;

      // This is the name of our downloaded file
      createEl.download = "Hasil PKL Riset 2 - Tujuan 1 - Hasil " + [i];
